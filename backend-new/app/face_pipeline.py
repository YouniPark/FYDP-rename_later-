...
from app.face_contracts import (
    FaceDebugIdentity,
    FaceDebugResponse,
    LatestFaceDecisionResponse,
    VideoFrameWsMessage,
)
...
async def _enqueue_frame(frame: VideoFrameWsMessage) -> None:
    ...
    await state.frame_queue.put(
        FaceFrameEnvelope(timestamp=frame.timestamp, jpeg_bytes=jpeg_bytes)
    )
...
def _encode_jpeg_b64(frame_bgr: np.ndarray) -> str | None:
    ok, encoded = cv2.imencode(
        ".jpg", frame_bgr, [int(cv2.IMWRITE_JPEG_QUALITY), 70]
    )
    if not ok:
        return None
    ...
    while True:
        ...
        try:
            ...
            prediction = await asyncio.to_thread(
                state.recognizer.predict_frame, frame
            )
    ...
    ws.onmessage = (evt) => {
      ...
      if (message.frame_jpeg_b64) {
        document.getElementById("frame").src =
          `data:image/jpeg;base64,${message.frame_jpeg_b64}`;
      }
    };
  </script>
</body>
